...
//            "</table>";
    // email alarm template
    private static final String mailBodyTemplate = TemplateUtil.getFailEmailTemplate();
        ...
        if (info != null && info.getAlarmEmail() != null && info.getAlarmEmail().trim().length() > 0) {
            ...
            for (String email : emailSet) {
                ...
                String content = MessageFormat.format(mailBodyTemplate, group != null ? group.getTitle() : "null", info.getId(), info.getJobDesc()
                        , info.getExecutorHandler(), info.getAuthor(), jobLog.getExecutorAddress(), jobLog.getTriggerMsg() + "," + jobLog.getHandleMsg());
